...
if (isset($_SESSION['role'])) {
    if ($_SESSION['role'] == 'Kepala Divisi' && $_SESSION['npk'] == '01033') {
        $queryMutasi = "SELECT *
                                ...
                                AND mutasi.cwocBaru IN ('QA', 'PDE 2W', 'PDE 4W', 'CQE 2W', 'CQE 4W')
                                AND mutasi.status = '10'";
    } elseif ($_SESSION['role'] == 'Kepala Divisi' && $_SESSION['npk'] == '01561') {
        $queryMutasi = "SELECT *
                        FROM mutasi
                        WHERE mutasi.hapus IS NULL
                        AND mutasi.cwocBaru IN ('HRD IR', 'GA', 'MIS') AND mutasi.status = '10'";
    } elseif ($_SESSION['role'] == 'Kepala Divisi' && $_SESSION['npk'] == '01961') {
        $queryMutasi = "SELECT *
                    FROM mutasi
                    WHERE hapus IS NULL
                    AND mutasi.cwocBaru IN ('PCE', 'PE 2W', 'PE 4W') AND mutasi.status = '10'";
    } elseif ($_SESSION['role'] == 'Kepala Divisi' && $_SESSION['npk'] == '01166') {
        $queryMutasi = "SELECT *
                      FROM mutasi
                      WHERE hapus IS NULL
                      AND mutasi.cwocBaru IN ('MARKETING', 'PROCUREMENT', 'VENDOR DEVELOPMENT', 'GENERAL PURCHASE') AND mutasi.status = '10'";
    } elseif ($_SESSION['role'] == 'Kepala Divisi' && $_SESSION['npk'] == '01266') {
        $queryMutasi = "SELECT *
                      FROM mutasi
                      WHERE hapus IS NULL
                      AND mutasi.cwocBaru IN ('WAREHOUSE', 'PRODUCTION SYSTEM', 'PPC') AND mutasi.status = '10'";
    } elseif ($_SESSION['role'] == 'Kepala Divisi' && $_SESSION['npk'] == '01577') {
        $queryMutasi = "SELECT *
                        FROM mutasi
                        WHERE hapus IS NULL
                        AND mutasi.cwocBaru IN ('PRODUCTION 1', 'PRODUCTION 2', 'PRODUCTION 3', 'PRODUCTION 4', 'PRODUCTION 5') AND mutasi.status = '10'
";
    } elseif ($_SESSION['role'] == 'Direktur Plant') {
        $queryMutasi = "SELECT *
                  FROM mutasi
                  WHERE mutasi.hapus IS NULL AND mutasi.status = '10' AND mutasi.cwocBaru IN ('PDE 2W', 'PDE 4W', 'QA', 'CQE 2W', 'CQE 4W', 'PCE', 'PE 2W', 'PE 4W', 'PRODUCTION 1', 'PRODUCTION 2', 'PRODUCTION 3', 'PRODUCTION 4', 'PRODUCTION 5')";
    } elseif ($_SESSION['role'] == 'Direktur Non Plant') {
        $queryMutasi = "SELECT *
                  FROM mutasi
                  WHERE hapus IS NULL AND mutasi.status = '10' AND mutasi.cwocBaru IN ('HRD IR', 'MIS', 'GA', 'MARKETING', 'PROCUREMENT', 'VENDOR DEVELOPMENT', 'GENERAL PURCHASE')";
    } else {
        $queryMutasi = "SELECT *
                FROM mutasi
                WHERE hapus IS NULL
                AND mutasi.status = '10'
                AND mutasi.cwocBaru = '{$_SESSION['dept']}'";
    }
}
...
<head>
    ...
    <title>Permohonan Mutasi | Mutasi Masuk</title>
