...
$IdMutasi = $_GET['IdMutasi']; // Ensure this is properly sanitized
$queryMutasi = "SELECT * FROM mutasi WHERE IdMutasi = '$IdMutasi'";
...
while ($row = $result->fetch_assoc()) {
    ...
    // Fetch sectAsalDesc from hrd_sect table
    ...
    $pdf->Cell(41, 5, 'Nama Karyawan             : ' . substr($fullName, 0, 15) . (strlen($fullName) > 15 ? '...' : ''), 0, 0);
    $pdf->Cell(35, 5, '', 'B', 0);
    $pdf->SetXY(88, 32);
    $pdf->Cell(15, 5, 'NPK   ' . $row['emno'], 0, 0);
    $pdf->SetXY(98, 31);
    $pdf->Cell(42, 5, '', 'B', 1);
    ...
    function getFullName($koneksi2, $npk)
    {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            return $npk; // Return empty string if npk is not found
        }
    }
